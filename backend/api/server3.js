...
async function initOraclePool() {
  if (!oraclePool && dbConfig.type === 'oracle') {
    try {
...
async function executeQuery(sql, params = {}, options = {}) {
  ...
  try {
    ...
    if (connection) {
      try {
        if (dbConfig.type === 'oracle') {
          // Release Oracle connection back to the pool
  ...
  try {
    ...
    if (isQueryOnly || (isProcedureThenQuery && procedureResult)) {
      ...
      if (dbConfig.type === 'oracle') {
        ...
        // If we have procedure output params, convert @param to :param bind variables
        if (procedureResult?.outBinds) {
          console.log('Using procedure output parameters:', procedureResult.outBinds);
          // Replace @param with :param bind variables for safe parameter binding
          for (const [key, value] of Object.entries(procedureResult.outBinds)) {
            const paramRegex = new RegExp(`@${key}\\b`, 'g');
            finalSqlToExecute = finalSqlToExecute.replace(paramRegex, `:${key}`);
            
            // Add to bind parameters with proper type handling
            const paramConfig = [...(endpoint.pathParams || []), ...(endpoint.queryParams || [])]
              .find(p => p.name === key);
            ...
            } else if (value instanceof Date || (paramConfig?.type === 'date')) {
              oracleBindParams[key] = {
                val: value instanceof Date ? value : String(value),
                type: oracledb.DATE
              };
            } else {
              oracleBindParams[key] = {
            ...
            console.log(`Bound output parameter @${key} as :${key} with value:`, value);
          }
          console.log('Modified SQL with bind variables:', finalSqlToExecute);
        }
        ...
        // If we have output parameters from the procedure, convert @param to :param bind variables
        if (procedureResult?.outBinds && Object.keys(procedureResult.outBinds).length > 0) {
          ...
          for (const [key, value] of Object.entries(procedureResult.outBinds)) {
            const paramRegex = new RegExp(`@${key}\\b`, 'g');
            mysqlSql = mysqlSql.replace(paramRegex, `:${key}`);
            ...
            mysqlParams[key] = processedValue;
            console.log(`Bound output parameter @${key} as :${key} with value:`, value);
          }
          console.log('Modified SQL with bind variables:', mysqlSql);
        }
        ...
        for (const paramName of uniqueParams) {
          // Skip parameters that were already handled as output parameters
          if (procedureResult?.outBinds && paramName in procedureResult.outBinds) continue;
    ...
    const resultData = queryResult?.rows || [];
    res.json({
      success: true,
      type: resultData.length > 0 && resultData[0]?.message?.toLowerCase() !== "success" ? "error" : "success",
      data: resultData
    });
    ...
    const formattedError = extractError(dbConfig.type, error);

    res.status(statusCode).json({
      success: false,
      type: "error",
      error: formattedError.message || 'Failed to execute endpoint',
    });
  } finally {
    if (connection) {
      try {
...
function parseOracleError(err) {
  ...
  const appMatch = msg.match(/ORA-20\d{3}:\s*(.*?)(?:\r?\n|$)/);
  if (appMatch) {
    return {
      type: 'APPLICATION_ERROR',
      code: msg.match(/ORA-20\d{3}/)?.[0],
      message: appMatch[1].trim()
    };
  }

  // Standard Oracle errors
  const stdMatch = msg.match(/ORA-\d{5}:\s*(.*?)(?:\r?\n|$)/);
  if (stdMatch) {
    return {
      type: 'ORACLE_ERROR',
      code: msg.match(/ORA-\d{5}/)?.[0],
      message: stdMatch[1].trim()
    };
  }
...
function extractError(dbType, err) {
  if (dbType === 'oracle') {
    return parseOracleError(err);
...
const upload = multer({
  ...
  limits: {
    fileSize: 50 * 1024 * 1024 // 50MB default limit, will be validated against DB config
  }
});

// Helper function to get document configuration from database
async function getDocumentConfig(clmDocCd) {
  let connection;
  try {
    connection = await getConnection();
    ...
    if (dbConfig.type === 'oracle') {
      result = await connection.execute(
        `SELECT clm_doc_cd, max_file_size, allowed_mime_types, upload_path 
         FROM giis_claim_document_config 
         WHERE clm_doc_cd = :clmDocCd`,
        { clmDocCd }
      );
    } else if (dbConfig.type === 'mysql') {
      const [rows] = await connection.execute(
    ...
    if (result?.rows?.[0]) {
      ...
      return {
        clmDocCd: config[0] || config.clm_doc_cd,
        maxFileSize: config[1] || config.max_file_size,
        allowedMimeTypes: config[2] || config.allowed_mime_types,
        uploadPath: config[3] || config.upload_path
      };
    }
    return null;
  } catch (error) {
...
// Helper function to save claim document record
async function saveClaimDocument(claimId, clmDocCd, fileName, filePath, fileSize, mimeType) {
  let connection;
  try {
    connection = await getConnection();
    ...
    if (dbConfig.type === 'oracle') {
      await connection.execute(
        `INSERT INTO giis_claim_documents (claim_id, clm_doc_cd, file_name, file_path, file_size, mime_type, uploaded_at)
         VALUES (:claimId, :clmDocCd, :fileName, :filePath, :fileSize, :mimeType, SYSDATE)`,
        {
          claimId,
          clmDocCd,
          fileName,
      ...
      await connection.execute(
        `INSERT INTO giis_claim_documents (claim_id, clm_doc_cd, file_name, file_path, file_size, mime_type, uploaded_at)
         VALUES (?, ?, ?, ?, ?, ?, NOW())`,
        [claimId, clmDocCd, fileName, filePath, fileSize, mimeType]
      );
      await connection.commit();
    }
  } catch (error) {
    if (connection && dbConfig.type === 'mysql') {
      await connection.rollback();
    }
    console.error('Error saving claim document:', error);
    throw error;
  } finally {
    if (connection) {
      try {
...
// File upload endpoint for claim documents
app.post('/uploads/claims/:claimId/documents', upload.array('files'), async (req, res) => {
  let connection;
  const uploadedFiles = [];
  const errors = [];
  ...
  try {
    ...
    // Handle clmDocCds in different formats:
    // 1. As JSON string: req.body.clmDocCds
    // 2. As array in form data: req.body.clmDocCds[] (multer handles this)
    // 3. As individual fields: req.body['clmDocCd_0'], req.body['clmDocCd_1'], etc.
    let clmDocCds = [];
    
    if (req.body.clmDocCds) {
      if (typeof req.body.clmDocCds === 'string') {
        try {
          clmDocCds = JSON.parse(req.body.clmDocCds);
        } catch (e) {
          clmDocCds = [req.body.clmDocCds];
        }
      } else if (Array.isArray(req.body.clmDocCds)) {
        clmDocCds = req.body.clmDocCds;
      } else {
        clmDocCds = [req.body.clmDocCds];
      }
      ...
      for (let i = 0; i < files.length; i++) {
        const fieldName = `clmDocCd_${i}`;
        if (req.body[fieldName]) {
          clmDocCds.push(req.body[fieldName]);
        }
      }
    }
    ...
    if (!clmDocCds || clmDocCds.length === 0) {
      return res.status(400).json({
        success: false,
        error: 'No claim document codes provided'
      });
    }
    
    if (files.length !== clmDocCds.length) {
      return res.status(400).json({
        success: false,
        error: `Number of files (${files.length}) must match number of claim document codes (${clmDocCds.length})`
      });
    }
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
      const clmDocCd = clmDocCds[i];
      
      try {
        // Get document configuration from database
        const docConfig = await getDocumentConfig(clmDocCd);
        
        if (!docConfig) {
          errors.push({
            file: file.originalname,
            clmDocCd,
            error: `Document configuration not found for code: ${clmDocCd}`
          });
          continue;
        }
        
        // Validate file size
        const maxSize = docConfig.maxFileSize || 10 * 1024 * 1024; // Default 10MB
        ...
        // Validate mime type
        const allowedMimeTypes = docConfig.allowedMimeTypes 
          ? (typeof docConfig.allowedMimeTypes === 'string' 
              ? docConfig.allowedMimeTypes.split(',').map(t => t.trim())
              : docConfig.allowedMimeTypes)
          : ['application/pdf', 'image/jpeg', 'image/png', 'image/jpg']; // Default allowed types
        
        if (!allowedMimeTypes.includes(file.mimetype)) {
          errors.push({
            file: file.originalname,
            clmDocCd,
            error: `Mime type ${file.mimetype} is not allowed. Allowed types: ${allowedMimeTypes.join(', ')}`
          });
          continue;
        }
        
        // Get upload path from database config
        const uploadPath = docConfig.uploadPath || path.join(process.cwd(), 'data', 'uploads');
        ...
        const fileName = `${claimId}_${clmDocCd}_${Date.now()}_${i}${fileExt}`;
        ...
        try {
          ...
          // If database save failed, clean up the file
          try {
    ...
    res.status(500).json({
      success: false,
      error: formattedError.message || 'Failed to upload files'
    });
  }
});

// Add graceful shutdown for Oracle and MySQL pools
process.on('SIGINT', async () => {
  if (dbConfig.type === 'oracle' && oraclePool) {
  ...
  } else if (dbConfig.type === 'mysql' && mysqlPool) {
...
const PORT = process.env.API_BUILDER_PORT || 3001;
app.listen(PORT, () => {
